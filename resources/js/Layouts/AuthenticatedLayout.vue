...
import {
  faList, faTags, faDoorOpen, faFileAlt, faLayerGroup, faUserCircle,
  ...
  <div class="h-screen flex flex-col">
    ...
    <header class="bg-gradient-to-r from-orange-400 to-orange-600 text-black shadow-md fixed top-0 w-full z-50">
      <div class="px-6 py-3 flex justify-between items-center">
        ...
        <button @click="isSidebarOpen = !isSidebarOpen" class="block md:hidden text-black mr-3" style="font-size: 150%;">
          <FontAwesomeIcon :icon="faList" />
        </button>
        <div class="text-2xl font-semibold text-black"
          style="text-shadow: 1px 1px 2px rgba(0, 0, 0, 2), -1px -1px 2px white; font-family: Porter Sans Block;">
          Dashboard
        </div>
        <h1 class="font-semibold text-center flex-wrap text-2xl text-black"
          style="text-shadow: 1px 1px 2px rgba(0, 0, 0, 2), -1px -1px 2px white; margin-left: 0%; font-family: Porter Sans Block;">
          TLAPALERÍA AKC
        </h1>
        <!-- Datos de la sesión -->
        <div class="flex items-center space-x-4">
          <div class="relative group">
            <div class="bg-orange-200 hover:bg-red-300" style="padding: 4px; border-radius: 7px;">
              <div style="background-color: #f48c1f; padding: 4px 3px; border-radius: 7px;">
                <FontAwesomeIcon :icon="faUserCircle" class="text-2xl" />
              </div>
            </div>
            <!-- Tooltip usuario -->
            <div class="absolute mt-5 w-80 bg-white text-black text-sm rounded-xl shadow-lg p-3 justify-center text-center opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-opacity duration-300"
          ...
          <div class="relative group">
            <div class="bg-orange-200" style="padding: 4px; border-radius: 7px;">
            ...
            <!-- Tooltip logout -->
            <div class="absolute left-1/2 transform -translate-x-1/2 mt-5 w-36 bg-white text-center text-black text-sm rounded-lg shadow-lg p-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-opacity duration-300"
    ...
    <div class="flex flex-1 pt-3">
      <aside :class="{'-translate-x-full md:translate-x-0': !isSidebarOpen, 'translate-x-0': isSidebarOpen}"
        class="fixed md:relative top-0 left-0 h-[calc(100vh-64px)] w-56 bg-gradient-to-b from-orange-400 to-orange-600 text-black transition-transform duration-300 ease-in-out shadow-lg rounded-xl z-40">
        <br>
        ...
        <div class="px-4 pt-1 text-2xl font-semibold flex ml-14 text-black">
          <span style="text-shadow: 1px 1px 2px rgba(0, 0, 0, 2), -1px -1px 2px white; font-family: Porter Sans Block;">MENÚ</span>
        </div>
        <nav class="mt-4">
          <ul>
            ...
            <li class="px-1 py-3 flex flex-col">
              ...
              <transition name="expand">
                <ul v-if="isOpen" class="w-full rounded-md shadow-lg overflow-hidden">
                  <li>
